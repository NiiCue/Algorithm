...
class Kingdom:
    def __init__(self):
        ...
        piece = ['wW', 'wA', 'wP', 'wC', 'eW', 'eA', 'eP', 'eC']
        type = [Warrior(), Archer(), Protector(), Commander(), Warrior(), Archer(), Protector(), Commander()]
        self.chess = {key: value for key, value in zip(piece, type)}
        self.player = 'w'
            ...
            if board[first_move[0]][first_move[1]] == '--':
                ...
                for dd in move:
                    ...
                    if board[second_move[0]][second_move[1]] == '--':
                        if [second_move[0],second_move[1]] not in vaild_move:
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                row = y // SQSIZE
                col = x // SQSIZE
                game.current_winner=game.winner()
                print(game.current_winner)
                # policies = game.vaild_policy()
                # print(len(policies))
                # for _ in policies:
                #     for rows in _.board:
                #         print(rows)
                ...
                if selected1 and selected2:
                    ...
                    if [row,col] not in vaild_atk:
                        ...
                        print('请选中')
                    game.player = 'e' if game.player == 'w' else 'w'
                    policies = game.vaild_policy()
                    # print(len(policies))
                    # for _ in policies:
                    #     for rows in _.board:
